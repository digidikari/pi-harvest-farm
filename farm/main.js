...
function loadAssets(callback) {
  ...
  fetch('data/lang.json')
    .then(response => {
      if (!response.ok) {
        console.error('Failed to fetch data/lang.json:', response.status, response.statusText);
        throw new Error('Failed to load data/lang.json');
      }
      console.log('Fetching data/lang.json successful');
    ...
    .then(data => {
      ...
      if (loadingText) {
        loadingText.innerText = langData[currentLang].loading || 'Loading...';
      } else {
        console.error('Loading text element not found');
      }
      const langBtn = document.getElementById('lang-btn');
      if (langBtn) {
        langBtn.innerText = langData[currentLang].switchLang || 'Switch Language (EN/ID)';
      }
      const gameTitle = document.getElementById('game-title');
      if (gameTitle) {
        gameTitle.innerText = langData[currentLang].title || 'Pi Harvest Farm';
      }
      const startBtn = document.getElementById('start-btn');
      if (startBtn) {
        startBtn.innerText = langData[currentLang].start || 'Start Game';
      }
      
      // Load vegetables data
      console.log('Fetching data/vegetables.json...');
      return fetch('data/vegetables.json');
    })
    .then(response => {
      if (!response.ok) {
        console.error('Failed to fetch data/vegetables.json:', response.status, response.statusText);
        throw new Error('Failed to load data/vegetables.json');
      }
      console.log('Fetching data/vegetables.json successful');
      return response.json();
    })
    .then(data => {
      ...
      const audioPromises = [
        ...
        loadAudio(mainSound, 'assets/sfx/voice/main-bgv.mp3'),
        loadAudio(menuSound, 'assets/sfx/voice/menu-bgv.mp3')
      ];
    ...
    .catch(error => {
      ...
      const loadingText = document.getElementById('loading-text');
      if (loadingText) {
        loadingText.innerText = 'Error loading assets. Please refresh.';
      }
    });
}
...
function loadAudio(audioElement, src) {
  return new Promise((resolve, reject) => {
    if (!audioElement) {
      console.error(`Audio element for ${src} not found`);
      reject(new Error(`Audio element for ${src} not found`));
      return;
    }
...
function updateWallet() {
  ...
  if (coinDisplay) coinDisplay.innerText = `${coins} Farm Coins`;
  if (waterDisplay) waterDisplay.innerText = `${water} Water`;
  if (piDisplay) piDisplay.innerText = `${pi.toFixed(2)} PI`;
  if (levelText) levelText.innerText = `${langData[currentLang].level || 'Level'}: ${level}`;
...
function playMainSound() {
  if (mainSound) {
    ...
    mainSound.play().catch(error => console.log('Main sound play failed:', error));
  }
}
...
function playMenuSound() {
  if (menuSound) {
    ...
    menuSound.play().catch(error => console.log('Menu sound play failed:', error));
  }
}
...
function renderInventory() {
  ...
  if (!inventorySection) {
    console.error('Inventory section element not found');
    return;
  }
...
function renderSellSection() {
  ...
  if (!exchangeSection) {
    console.error('Exchange section element not found');
    return;
  }
...
function renderShop() {
  ...
  if (!shopSection) {
    console.error('Shop section element not found');
    return;
  }
...
function renderUpgrades() {
  ...
  if (!upgradesSection) {
    console.error('Upgrades section element not found');
    return;
  }
...
function renderAchievements() {
  ...
  if (!achievementsSection) {
    console.error('Achievements section element not found');
    return;
  }
...
function initializeGame() {
  ...
  loadAssets(() => {
    ...
    // Coba mainkan background music, tapi jangan biarkan error bikin stuck
    if (backgroundMusic) {
      backgroundMusic.play().catch(error => {
        console.log('Autoplay blocked for background music, user interaction required:', error);
      });
    }
    playMainSound();
    ...
    if (lastClaim !== now && dailyRewardModal) {
...
if (sfxVolumeInput) {
  sfxVolumeInput.addEventListener('input', (e) => {
    if (buyingSound) buyingSound.volume = e.target.value;
    if (wateringSound) wateringSound.volume = e.target.value;
    if (harvestSound) harvestSound.volume = e.target.value;
    if (upgradeSound) upgradeSound.volume = e.target.value;
    if (mainSound) mainSound.volume = e.target.value;
    if (menuSound) menuSound.volume = e.target.value;
  });
}
